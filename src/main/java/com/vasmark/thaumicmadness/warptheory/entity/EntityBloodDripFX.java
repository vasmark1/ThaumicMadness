...
public class EntityBloodDripFX extends EntityFX {

    private int bobTimer;

    public EntityBloodDripFX(World world, double x, double y, double z, float particleRed, float particleGreen,
        float particleBlue) {
    ...
    public EntityBloodDripFX(World world, double x, double y, double z, float particleRed, float particleGreen,
        ...
        this.particleGravity = -0.06F * (float) gravityMod;
        this.bobTimer = 40;
    ...
    public void onUpdate() {
        ...
        if (this.particleGravity > 0.0F) {
            material = this.worldObj
                .getBlock((int) Math.floor(this.posX), (int) Math.floor(this.posY), (int) Math.floor(this.posZ))
                .getMaterial();
            if (material.isLiquid() || material.isSolid()) {
                d0 = Math.floor(this.posY) + 1.0D
                    - (double) BlockLiquid.getLiquidHeightPercent(
            ...
            material = this.worldObj
                ...
                .getMaterial();
            if (material.isLiquid() || material.isSolid()) {
                d0 = (double) ((float) ((int) Math.ceil(this.posY) + 1) - BlockLiquid.getLiquidHeightPercent(
                    this.worldObj.getBlockMetadata(
